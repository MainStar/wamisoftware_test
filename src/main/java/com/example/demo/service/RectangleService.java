...
import static com.example.demo.utils.Constants.RECTANGLE_SHAPE_TYPE;
...
public class RectangleService implements ShapeService {
  ...
  public ShapeResponseDto calculate(ShapeRequestDto requestDto) throws ValidationCheckException {
    ...
    responseDto.setShapeType(RECTANGLE_SHAPE_TYPE);
  ...
  private void validateFields(BigDecimal sideA, BigDecimal sideB) throws ValidationCheckException {
    if (sideA == null) {
      throw new ValidationCheckException("SideA field should be provided for calculating rectangle are");
    }
    if (sideB == null) {
      throw new ValidationCheckException("SideB field should be provided for calculating rectangle are");
    }
  }
}
